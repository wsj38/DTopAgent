#!/usr/bin/env python3
"""
Short Generation Script for AdaComp SPARK

This script generates short, concise answers using a language model pipeline.
It processes questions with document context and generates brief responses.
...
def get_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate short, concise answers using language model pipeline"
    )
    parser.add_argument(
        '--model_path', 
        type=str, 
...
def create_messages(question: str, meta_list: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    """
    Create conversation messages for short answer generation.
    ...
    user_input = (
        ...
        f"Return only the final answer without explanation or extra words.\n\n"
        f"Documents:\n{formatted_docs}\n\n"
        f"Question: {question}"
    )
    ...
    messages = [
        {
            ...
            "content": "You are a helpful, respectful and honest assistant, "
                      "and please use documents provided to answer the question."
        },
...
def generate_answer(
    ...
    """
    Generate a short answer using the language model pipeline.
...
def main() -> None:
    """
    Main function to run the short generation pipeline.
    """
    args = get_args()
